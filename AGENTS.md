...
- [x] **Demo AI humanization (v0.13.x) & look-ahead prediction (v0.14.x)** — `src/entities/ai.js` + `src/entities/asteroid.js`. Three generations of humanization features progressively close the gap between robotic instant-tracking and life-like piloting. v0.13.x adds 5 features that model the limiting factors of human piloting (sensor delay, intent commitment, decision granularity) plus 2 physical-behavior tightenings (close-range coast-in, smart-wander gap-awareness). v0.14.x adds the 3rd generation: target-prediction look-ahead.
