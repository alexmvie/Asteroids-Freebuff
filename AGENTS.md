...
- [x] **v0.56.0–v0.58.0 — Pirate visual foundation + tuner UI reconciliation (COMPRESSED)** — spawned two persistent pirate entities with a procedural hazard-stripe texture (extending v0.55.0's open `BEHAVIORS` registry seam with a `pirate` behavior) and reconciled the AI Tuners panel back to the slimmed-down `AI_TUNABLES` bag. The pirate behavior tuning (`aggroDist`) and the procedural texture helper are both still wired in; the AI combat loop that gives them agency lands in v0.60.0 (see pirate combat entry below).
